...
连续性采用保守契约：`continue_day_cnt` 为正整数，并与明确 `首板`（要求1）、`N连板` 或 `N天N板` 文本一致才计连续板。`5天4板`、首板计数0、缺文本、数值冲突均标记未确认，不猜测0/1约定。主口径存在未确认连续性时，首板/连板/最高板/板位分布以及依赖它们的热度保留缺失，涨停参与数仍可用。晋级按各档昨日名单独立校验，可能在全市场连板数未确认时仍然有效。真实首板计数含义尚须按下述验证步骤核对。
...
真实相邻交易日 T-1、T，按代码匹配。1→2、2→3、3→4 及连板总体：以前日相应连续板位（总体为≥2）的主口径集合为分母，今日仍在主口径且连续高度恰为原高度+1为分子。返回来源/目标日期、完整性、分子分母、晋级与未晋级代码。不跨缺口使用前一个“有数据日”。未晋级不等于跌停/亏损。

四档的 `complete` 分别判定，不共用全池 `boards_complete`：

1. 先确认昨日名单。明确排除主口径（任一ST/未开板新股标记为true已足够），或连续高度明确不属该档的记录不影响该档。其余记录若范围或连续高度未知、不能排除属于该档，则该档分母无法确认，不能丢弃未知后声称名单完整。例如昨日已确认首板但范围未知，只影响1→2，不影响2→3/3→4/multi；昨日范围有效但连续高度未知则可能影响所有档位。
2. 仅核验昨日名单内股票在今日完整池中的结果。不在今日池，或今日明确被主口径排除，均是已确认未晋级，后者无需连续高度有效。今日属于主口径且高度明确则按是否等于昨日+1判定；名单内范围或高度仍未知则仅该档不完整。今日名单外股票的未知字段不参与该档校验。

昨日A/B均为主口径二板，今日A三板、B不在池中，新增X为“5天4板”或范围未知：2→3仍为完整1/2，晋级[A]、未晋级[B]。全市场连板计数可因X未知而缺失，不会清空这个已确认晋级率。

昨日源 `None` 表示缺失，各档不完整；昨日合法空池 `[]` 的各档完整、分子分母为0、比例null。昨日非空名单遇到今日完整空池，晋级率可确认是0。不完整档的分子/分母/比例为null，两个名单均不输出，未知股票不会冒充未晋级。
...
代码更新不会修改已保存的旧聚合。此前因无关未知记录被清空的晋级率需要重算。现有 `publish` 路径已验证：重新发布旧日期会从持久化完整源按时间顺序重算该日及所有已保存后续日，不用改表或迁移。

由管理员调用 `POST /api/v1/market-sentiment/run`，提交 `{"trade_date":"最早受影响交易日（YYYY-MM-DD）"}`。该日核心取数成功后，会应用修复并重算后续；若取数失败，则保持旧结果。最近31交易日也可用 `{"backfill_days":31,"missing_only":false}` 强制刷新；默认 `missing_only=true` 会跳过既有成功日期，不能用于保证修复旧聚合。此处仅说明操作，本次未运行生产重算或部署。
...
初次实现验证结果：后端完整 `ci_gate.sh` 通过（1876 passed、21 skipped、92 subtests），其中市场情绪专项56项全部通过，包含独立 Docker PostgreSQL 16 的真实迁移与原子发布验证。Web Vitest 467项通过，lint 0 errors（仓库既有380 warnings），build成功。新增 Playwright 在1280/1440/1920宽度×浅深主题6项通过，并检查截图。线上验证脚本实际执行后因缺少 `FUYAO_API_KEY` 返回2，未发真实请求。

晋级有效性修复验证：旧算法被新增回归复现11处失败；修复后市场情绪专项73项通过（含独立PostgreSQL和publish→历史重算→overview/history API回归），完整后端门禁1893 passed、21 skipped、92 subtests。未修改前端，本次未重跑Web测试。再次执行现有协议脚本因后端未配置 `FUYAO_API_KEY` 返回2，没有真实样本可统计连续性未确认数量/原因；真实协议仍未验证，未调整任何字段适配。全市场 `boards_complete` 及连续20交易日要求不变，未知连续性仍可能造成当日和后续20交易日热度缺失。
