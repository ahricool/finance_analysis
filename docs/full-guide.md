...
>
> 从 `python main.py --schedule`、`python main.py --serve --schedule` 或等价内置调度模式启动后，WebUI 保存新的 `SCHEDULE_TIME` 会在 APScheduler 的时间同步任务（默认约每 30 秒）内自动重绑 daily job，无需重启进程；旧的执行时间不会继续保留。
