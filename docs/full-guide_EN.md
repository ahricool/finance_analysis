...
>
> When the built-in scheduler is started via `python main.py --schedule`, `python main.py --serve --schedule`, or an equivalent local mode, saving a new `SCHEDULE_TIME` from the WebUI will rebind the daily job on the next APScheduler schedule-time sync cycle (about every 30 seconds) without restarting the process. The previous trigger time is removed instead of being kept alongside the new one.
