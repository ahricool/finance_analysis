...
3. 股票键为 `(symbol, range_days)`；游资明细键为 `(游资名称, symbol, range_days)`。
   ...
   扶摇也会返回10/30日异常波动榜：先按完整响应核对数量、校验字段与重复冲突，再将这些记录
   分离到每日来源 JSONB 的 `excluded_rows` / `excluded_details`，保留规范化金额和概念供审计。
   它们不进入1日累计、3日截面、游资展示或共振评分，也不会导致正常1/3日记录整日拒收。
   `quality.excluded_period_counts` 按周期记录去重后的排除条数，并经 `source_quality` 返回；
   游资来源按有限明细条数统计，不将其与上游股票数量混用。原始 `upstream_count` / `upstream_stock_count`
   保持不变。只有10/30日记录时，1/3日观察为已完整采集的空集；其他未知周期仍拒绝。
...
页面与 overview API 默认 `days=1`，可切换1/5/10/20日；3日榜仍是独立截面。
全部股票表不受所选概念限制，支持代码/名称/概念搜索及金额排序，点击查看当时逐日证据。
游资席位样本独立展开，列出全部返回记录，不限图中的Top5。
缺日窗口的严格区间总额仍为null；股票明细单独展示已采集日合计并明确标注，不能当作完整区间收益或资金总额。
未知机构/游资金额仍为null。概念分摊与股票原始净额分别处理，不重复计数。
