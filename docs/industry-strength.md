...
成分 endpoint 按当前行业目录判断行业是否存在，不要求它出现在最新 ranking；目录中不存在返回 404，扶摇错误返回安全 503，可独立重试。
公开接口不返回用于审计的 `quality.member_codes`。

## 页面

1. 标题、日期选择、刷新，以及紧凑数据状态栏（收盘快照、实际有效日期、生成时间、覆盖数、必要警告）。口径说明可展开。
2. 四张摘要卡：最强行业、加速最快、动量降速最大、有效行业上涨占比。前三张打开统一详情抽屉；摘要始终基于当日有效行业截面。
...
7. 统一右侧详情抽屉：概览 / 历史表现 / 当前成分股。当前成分按需加载，日期与历史快照隔离。初次加载不自动打开。
