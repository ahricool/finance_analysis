...
- 每次定时运行直接通过 `MarketDataService` 请求当前目录、成分名单、行业指数/沪深300完整历史窗口，
  个股历史日 K 按去重代码优先读 DB，窗口不足时复用正式任务的 API fallback；Preview 今日临时线通过行情门面限定扶摇批量个股快照（每批最多100只），不走 Streaming / Longbridge 通用报价链。扶摇缺失时保留缺失语义，不切换其他来源。不缓存计算输入；上次数据不完整不会阻止下次重新获取。
