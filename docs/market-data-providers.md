...
`FuyaoProvider._get()` 请求：连接错误、超时、HTTP 408/429/5xx 与业务码 4001 按 2、4、8 秒指数退避，
...
easyquotation 的每个 HTTP GET 批次遇到连接错误或超时时，最多额外重试 3 次，
分别等待 2 秒、4 秒、8 秒；每次请求保持 30 秒超时。仅重试失败批次，不重新执行整轮
预演计算。HTTP 408/429/5xx 同样重试；证书错误、其他 HTTP 错误和数据解析错误不重试。重试耗尽后
保持原有失败传播与预演状态处理，不增加 Celery 自动重试或其他行情源降级。

### 统一外部请求重试

`core/retry.py` 提供同步/异步的请求边界重试：首次调用后最多追加 3 次，
退避固定为 2、4、8 秒；失败耗尽保持调用方原来的错误/降级语义。
覆盖腾讯、扶摇、Alpaca、Yahoo、TickFlow、AkShare/Sina、指数成分、
Longbridge 行情/新闻/日历，以及 Binance REST。HTTP 连接/读取错误、超时、
408/429/5xx 可重试；鉴权、参数、证书和数据解析错误通常不重试。
SDK 通过异常类型和有限网络错误标记识别临时失败；TickFlow 与 LiteLLM 内置请求重试关闭，避免叠加；SDK 自身协议重连不属于应用层请求计数。
Yahoo 吞掉错误的批请求以返回的失败/缺失标的重试，保留已成功标的，默认最多 3 次；
`MARKET_DATA_YFINANCE_MAX_RETRIES` 可下调但不能超过 3。日线原有的数据补缺逻辑继续保留。
扶摇每次重试重新检查剩余总预算，预算不足则立即终止。
现有定时任务的数据就绪重试、日线补缺和 Streamer 长连接重连仍是独立业务机制。
浏览器 Binance REST 同样最多重试 3 次（2/4/8 秒），切换周期或卸载时取消请求及退避等待。
