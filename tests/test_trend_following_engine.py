...
def _previous(**updates):
    previous = {
        ...
        "pending_action": None,
        "pending_since": None,
        "pending_regime": "RISK_ON",
        "pending_max_exposure": 1.0,
    }
...
def test_regime_exposure_cap_blocks_excess_entry_and_add():
    ...
    desired = {
        row["code"]: transition_state(row, pending, trade_date=TRADE_DATE, market_regime="NEUTRAL") for row in rows
    }
    ...
    add_desired = {
        row["code"]: transition_state(row, holding, trade_date=TRADE_DATE, market_regime="RISK_ON") for row in add_rows
    }
    assert all(item.action == "ADD" for item in add_desired.values())
    add_gated = apply_exposure_gate(
        add_rows,
        add_desired,
        {row["code"]: holding for row in add_rows},
        max_exposure=0.075,
    )
...
def test_add_execution_can_generate_reduce_and_reduce_can_repeat():
    ...
    prior_reduce = _previous(
        ...
        initial_stop=80.0,
        trailing_stop=80.0,
        next_add_price=120.0,
        pending_action="REDUCE",
        pending_since=TRADE_DATE - timedelta(days=1),
        pending_regime="RISK_OFF",
        pending_max_exposure=0.0,
    )
    reduced = transition_state(
