...
from .constants import (
    ...
    EXPIRES_HOLDINGS_SYNC,
    EXPIRES_PORTFOLIO_RISK,
    EXPIRES_INTRADAY,
...
SCHEDULED_TASK_DEFINITIONS = (
    ...
    ScheduledTaskDefinition(
        job_id=JOB_HOLDINGS_SYNC,
        name="Google Sheet 持仓同步",
        description="每 5 分钟读取已连接的 Google Sheet 持仓快照；Sheet 是唯一持仓编辑源",
        task_type="scheduled_holdings_sync",
        celery_task_name=celery_task_name(JOB_HOLDINGS_SYNC),
        schedules=(CronSchedule(minute="*/5"),),
        schedule_text="每 5 分钟",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_INGESTION,
        expires=EXPIRES_HOLDINGS_SYNC,
    ),
    ScheduledTaskDefinition(
        job_id=JOB_PORTFOLIO_RISK_CN,
        name="A股持仓分钟风控",
        description="交易时段每分钟评估 A 股持仓硬保护与 5m 软退出；CN 5m 仅用新浪，通知走全局渠道",
        task_type="scheduled_portfolio_risk_cn",
        celery_task_name=celery_task_name(JOB_PORTFOLIO_RISK_CN),
        schedules=(
            CronSchedule(minute="*", hour="9-11", day_of_week="mon-fri"),
            CronSchedule(minute="*", hour="13-14", day_of_week="mon-fri"),
            CronSchedule(minute="0-5", hour="15", day_of_week="mon-fri"),
        ),
        schedule_text="A股交易日 09:30-11:30、13:00-15:05 每分钟 Asia/Shanghai（午休不运行）",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ALERTS,
        expires=EXPIRES_PORTFOLIO_RISK,
    ),
    ScheduledTaskDefinition(
        job_id=JOB_PORTFOLIO_RISK_US,
        name="美股持仓分钟风控",
        description="交易时段每分钟评估美股持仓硬保护与 5m 软退出；US 5m 仅用 yfinance，通知走全局渠道",
        task_type="scheduled_portfolio_risk_us",
        celery_task_name=celery_task_name(JOB_PORTFOLIO_RISK_US),
        schedules=(
            CronSchedule(minute="*", hour="9-16", day_of_week="mon-fri", timezone=US_TIMEZONE),
        ),
        schedule_text="美股交易日 09:30-16:05 America/New_York 每分钟（自动适配夏令时/半日市）",
        timezone=US_TIMEZONE,
        queue=QUEUE_ALERTS,
        expires=EXPIRES_PORTFOLIO_RISK,
    ),
