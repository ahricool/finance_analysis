...
from .constants import (
    ...
    EXPIRES_ETF_ROTATION_PREVIEW,
    EXPIRES_TREND_FOLLOWING,
    EXPIRES_TREND_FOLLOWING_PREVIEW,
    EXPIRES_INTRADAY,
    ...
    JOB_DAILY_ANALYSIS,
    JOB_MARKET_STRUCTURE_CN,
    JOB_MARKET_STRUCTURE_US,
    JOB_ETF_ROTATION_CN,
    JOB_ETF_ROTATION_US,
...
SCHEDULED_TASK_DEFINITIONS = (
    ...
    ScheduledTaskDefinition(
        ...
        schedule_text="每天 07:00 America/New_York",
    ...
    ScheduledTaskDefinition(
        ...
        schedules=(CronSchedule(minute="0", hour="8", timezone=US_TIMEZONE),),
        schedule_text="每天 08:00 America/New_York",
        timezone=US_TIMEZONE,
        queue=QUEUE_INGESTION,
    ...
    ScheduledTaskDefinition(
        ...
        schedules=(CronSchedule(minute="0", hour="9", timezone=US_TIMEZONE),),
        schedule_text="每天 09:00 America/New_York",
        timezone=US_TIMEZONE,
        queue=QUEUE_ANALYSIS,
    ...
    ScheduledTaskDefinition(
        ...
        name="美股盘中分析",
        description="每30分钟使用实时行情检测自选美股、QQQ及板块异动，按信号状态变化提醒",
        task_type="scheduled_us_intraday",
        celery_task_name=celery_task_name(JOB_US_INTRADAY_ANALYSIS),
        schedules=(
            CronSchedule(minute="45", hour="9", day_of_week="mon-fri", timezone=US_TIMEZONE),
            CronSchedule(minute="15,45", hour="10-15", day_of_week="mon-fri", timezone=US_TIMEZONE),
        ),
        schedule_text="美股交易日 09:45-15:45 America/New_York，每30分钟",
        timezone=US_TIMEZONE,
        queue=QUEUE_ALERTS,
    ...
    ScheduledTaskDefinition(
        ...
        schedules=(CronSchedule(minute="0", hour="18", timezone=US_TIMEZONE),),
        schedule_text="美股交易日 18:00 America/New_York",
        timezone=US_TIMEZONE,
        queue=QUEUE_ANALYSIS,
    ...
    ScheduledTaskDefinition(
        ...
        schedule_text="周一至周五 18:00 增量；每月第一个周日 10:00 全量（Asia/Shanghai）",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_INGESTION,
        expires=EXPIRES_MARKET_DATA_SYNC,
        allow_manual_run=True,
    ...
    ScheduledTaskDefinition(
        job_id=JOB_MARKET_STRUCTURE_CN,
        name="市场结构 CN",
        description="读取收盘日线与正式 ETF 排名，保存市场广度、轮动速度和领导集中度",
        task_type="scheduled_market_structure_cn",
        celery_task_name=celery_task_name(JOB_MARKET_STRUCTURE_CN),
        schedules=(CronSchedule(minute="50", hour="18", day_of_week="mon-fri", timezone=SCHEDULE_TIMEZONE),),
        schedule_text="周一至周五 18:50 Asia/Shanghai",
        timezone=SCHEDULE_TIMEZONE, queue=QUEUE_ANALYSIS, expires=EXPIRES_ETF_ROTATION, allow_manual_run=True,
    ),
    ScheduledTaskDefinition(
        job_id=JOB_MARKET_STRUCTURE_US,
        name="市场结构 US",
        description="读取收盘日线与正式 ETF 排名，保存市场广度、轮动速度和领导集中度",
    ...
    ScheduledTaskDefinition(
        job_id=JOB_ETF_ROTATION_PREVIEW_CN,
        name="A股 ETF 动量轮动盘中预演",
        description="交易日盘中用腾讯 ETF 实时行情构造当日临时日线，复用正式轮动计算且不写入正式快照",
        task_type="scheduled_etf_rotation_preview_cn",
        celery_task_name=celery_task_name(JOB_ETF_ROTATION_PREVIEW_CN),
        schedules=(
    ...
    ScheduledTaskDefinition(
        job_id=JOB_ETF_ROTATION_PREVIEW_US,
        name="美股 ETF 动量轮动盘中预演",
        description="交易日盘中用 Yahoo 5 分钟 K 线聚合当日临时日线，复用正式轮动计算且不写入正式快照",
        task_type="scheduled_etf_rotation_preview_us",
        celery_task_name=celery_task_name(JOB_ETF_ROTATION_PREVIEW_US),
        schedules=(
            CronSchedule(minute="5", hour="11", day_of_week="mon-fri", timezone=US_TIMEZONE),
            CronSchedule(minute="5", hour="15", day_of_week="mon-fri", timezone=US_TIMEZONE),
            CronSchedule(minute="35", hour="15", day_of_week="mon-fri", timezone=US_TIMEZONE),
        ),
        schedule_text="周一至周五 11:05、15:05、15:35 America/New_York",
        timezone=US_TIMEZONE,
        queue=QUEUE_ANALYSIS,
        expires=EXPIRES_ETF_ROTATION_PREVIEW,
        allow_manual_run=True,
    ...
    ScheduledTaskDefinition(
        ...
        schedule_text="周一至周五 18:40 Asia/Shanghai",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ANALYSIS,
    ...
    ScheduledTaskDefinition(
        job_id=JOB_TREND_FOLLOWING_PREVIEW_CN,
        name="A股趋势跟踪盘中预演",
        description="交易日盘中用腾讯全市场快照构造当日临时日线，复用正式趋势跟踪计算且不写入正式快照",
        task_type="scheduled_trend_following_preview_cn",
        celery_task_name=celery_task_name(JOB_TREND_FOLLOWING_PREVIEW_CN),
        schedules=(
            CronSchedule(minute="0", hour="11", day_of_week="mon-fri"),
            CronSchedule(minute="0", hour="14", day_of_week="mon-fri"),
            CronSchedule(minute="30", hour="14", day_of_week="mon-fri"),
        ),
        schedule_text="周一至周五 11:00、14:00、14:30 Asia/Shanghai",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ANALYSIS,
        expires=EXPIRES_TREND_FOLLOWING_PREVIEW,
        allow_manual_run=True,
    ),
    ScheduledTaskDefinition(
        job_id=JOB_TREND_FOLLOWING_PREVIEW_US,
        name="美股趋势跟踪盘中预演",
        description="交易日盘中用 Yahoo 5 分钟 K 线聚合当日临时日线，复用正式趋势跟踪计算且不写入正式快照",
        task_type="scheduled_trend_following_preview_us",
        celery_task_name=celery_task_name(JOB_TREND_FOLLOWING_PREVIEW_US),
        schedules=(
            CronSchedule(minute="0", hour="11", day_of_week="mon-fri", timezone=US_TIMEZONE),
            CronSchedule(minute="0", hour="15", day_of_week="mon-fri", timezone=US_TIMEZONE),
            CronSchedule(minute="30", hour="15", day_of_week="mon-fri", timezone=US_TIMEZONE),
        ),
        schedule_text="周一至周五 11:00、15:00、15:30 America/New_York",
        timezone=US_TIMEZONE,
        queue=QUEUE_ANALYSIS,
        expires=EXPIRES_TREND_FOLLOWING_PREVIEW,
        allow_manual_run=True,
    ...
    ScheduledTaskDefinition(
        ...
        name="A股盘中分析",
        description="每小时初筛A股全市场并精析有限候选，按信号状态变化提醒",
        task_type="scheduled_a_share_intraday",
        celery_task_name=celery_task_name(JOB_A_SHARE_INTRADAY_ANALYSIS),
        schedules=(
            CronSchedule(minute="45", hour="9-10", day_of_week="mon-fri"),
            CronSchedule(minute="0", hour="13-15", day_of_week="mon-fri"),
        ),
        schedule_text="A股交易日 09:45、10:45、13:00、14:00、15:00 Asia/Shanghai（午休不运行）",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ALERTS,
        expires=EXPIRES_INTRADAY,
    ),
    ScheduledTaskDefinition(
        job_id=JOB_A_SHARE_PRE_CLOSE_REVIEW,
        name="A股收盘前持仓与市场复核",
        description="交易日14:30复核A股市场、强势板块和当前持仓，给出比例化调整建议",
        task_type="scheduled_a_share_pre_close_review",
        celery_task_name=celery_task_name(JOB_A_SHARE_PRE_CLOSE_REVIEW),
        schedules=(CronSchedule(minute="30", hour="14", day_of_week="mon-fri"),),
        schedule_text="A股交易日 14:30 Asia/Shanghai",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ANALYSIS,
        expires=EXPIRES_PRE_CLOSE_REVIEW,
    ),
)
