...
SCHEDULED_TASK_DEFINITIONS = (
    ...
    ScheduledTaskDefinition(
        ...
        schedules=(CronSchedule(minute="0", hour="22", timezone=US_TIMEZONE),),
        schedule_text="美股交易日 22:00 America/New_York（日线同步后）",
        timezone=US_TIMEZONE,
        queue=QUEUE_ANALYSIS,
    ...
    ScheduledTaskDefinition(
        ...
        name="A股交易引擎",
        description="Beat 每 30 分钟检查一次；仅当 A 股处于常规交易时段才分析当前持仓。中线决策，不是全市场 Scanner，不自动下单",
        task_type="scheduled_trade_engine_cn",
        celery_task_name=celery_task_name(JOB_TRADE_ENGINE_CN),
        schedules=(CronSchedule(minute="*/30", day_of_week="mon-fri"),),
        schedule_text="每 30 分钟 Asia/Shanghai；不在 A 股常规交易时段则跳过",
        timezone=SCHEDULE_TIMEZONE,
        queue=QUEUE_ALERTS,
    ...
    ScheduledTaskDefinition(
        ...
        name="美股交易引擎",
        description="Beat 每 30 分钟检查一次；仅当美股处于常规交易时段才分析当前持仓。中线决策，不是全市场 Scanner，不自动下单",
        task_type="scheduled_trade_engine_us",
        celery_task_name=celery_task_name(JOB_TRADE_ENGINE_US),
        schedules=(CronSchedule(minute="*/30", day_of_week="mon-fri", timezone=US_TIMEZONE),),
        schedule_text="每 30 分钟 America/New_York；不在美股常规交易时段则跳过（自动适配夏令时/半日市）",
        timezone=US_TIMEZONE,
        queue=QUEUE_ALERTS,
        expires=EXPIRES_TRADE_ENGINE,
    ),
