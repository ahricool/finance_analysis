...
class AShareIntradayDataSource:
    ...
    def fetch_minute_bars(
        ...
        *,
        ...
        if interval == 1:
            minimum_count = MIN_BARS_FOR_BENCHMARK if code in A_SHARE_INDICES else MIN_BARS_FOR_SYMBOL
            try:
    ...
    def get_quote(self, code: str) -> Optional[UnifiedRealtimeQuote]:
        try:
            realtime_quote = self.realtime.get_quote(code, market_type="CN")
        except Exception as exc:
            realtime_quote = None
            logger.warning("symbol=%s source=market_streamer fallback_reason=redis_error error=%s", code, exc)
        ...
        try:
            ...
            if quote is not None:
                logger.info("symbol=%s source=fallback fallback_reason=longbridge_empty", code)
            return quote
        except Exception as exc:
