"""Independent Celery entry point for US signal evaluation."""
...
DEFINITION = require_scheduled_task_definition(JOB_SIGNAL_EVALUATION_US)
...
@track_task(
    ...
    record_result=True,
    success_message="美股信号评价完成",
    strip_lifecycle_kwargs=True,
)
def evaluate_signals_us(scheduler_job_id: Optional[str] = None, **_: Any) -> dict[str, Any]:
    now = datetime.now(ZoneInfo(DEFINITION.timezone))
    return SignalEvaluationService().evaluate_signals(market="US", now=now)


__all__ = ["evaluate_signals_us"]
