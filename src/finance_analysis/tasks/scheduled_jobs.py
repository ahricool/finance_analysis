...
from finance_analysis.tasks.celery.schedule import QUEUE_INGESTION, SCHEDULE_TIMEZONE, US_TIMEZONE
...
def _submit_market_calendar_importance_task(event_ids: Sequence[int]) -> None:
    ids = [int(event_id) for event_id in event_ids if event_id is not None]
    if not ids:
        return
    try:
        from finance_analysis.tasks.celery.jobs.market_calendar import market_calendar_importance
...
def run_market_calendar() -> dict:
    ...
    try:
        ...
        _submit_market_calendar_importance_task(importance_candidate_ids)
        logger.info(
            "美股财经日历任务完成: fetched=%s inserted=%s updated=%s duplicate=%s notify=%s importance_candidates=%s",
