"""Frozen Alpha V1 reference, imported only for opt-in debug comparison."""
...
def calculate_trend_score(row: dict[str, Any]) -> tuple[float, dict]:
    components = {
...
def calculate_rs_score(row: dict[str, Any]) -> tuple[float, dict]:
    components = {
...
def calculate_alpha_score(row: dict[str, Any]) -> tuple[float, dict]:
    components = {
    ...
    score = sum(
        components[key] * weight
        for key, weight in {"trend": 0.35, "rs": 0.30, "breakout": 0.20, "volume": 0.10, "compression": 0.05}.items()
    )
    return round(clamp(score), 4), components
...
def compare_score(row: dict[str, Any]) -> dict:
    legacy = dict(row)
    legacy["trend_score"], trend = calculate_trend_score(legacy)
    legacy["rs_score"], rs = calculate_rs_score(legacy)
    legacy["breakout_score"], breakout = calculate_breakout_score(legacy)
    score, alpha = calculate_alpha_score(legacy)
    return {"score": score, "trend": trend, "rs": rs, "breakout": breakout, "alpha": alpha}
