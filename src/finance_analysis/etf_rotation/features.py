"""Pure point-in-time fast-rotation features over daily bars."""
...
def calculate_features(
    ...
    """Calculate fast-rotation features from bars ordered oldest to newest.

    The current close is only compared with current or earlier sessions.  A
    minimum of 21 bars is required so every configured return and risk metric
    is available without future data.
    """
    ...
    previous_3d = closes[-4] / closes[-7] - 1.0
    ...
    regressions = {
        window: _weighted_log_regression(
            closes[-window:], config.regression_oldest_weight, config.regression_latest_weight
        )
        for window in config.regression_windows
    }
    slope_5d, _ = regressions[5]
    slope_10d, _ = regressions[10]
    slope_15d, r2_15d = regressions[15]
    annualized_5d = _annualize_slope(slope_5d, config.annualized_slope_clip)
    annualized_10d = _annualize_slope(slope_10d, config.annualized_slope_clip)
    annualized_15d = _annualize_slope(slope_15d, config.annualized_slope_clip)
    efficiency_changes = [
    ...
    efficiency = (
        (closes[-1] - closes[-1 - config.efficiency_window]) / efficiency_denominator
        if efficiency_denominator > 0 else 0.0
    )
    return FeatureSet(
        reference_price=current,
        **{f"ret_{window}d": returns[window] for window in RETURN_WINDOWS},
        previous_3d_return=previous_3d,
        previous_5d_return=previous_5d,
        momentum_acceleration_3d=returns[3] - previous_3d,
        momentum_acceleration_5d=returns[5] - previous_5d,
        ma10_ratio=current / ma10 - 1.0,
        ...
        weighted_slope_5d=slope_5d,
        weighted_slope_10d=slope_10d,
        weighted_slope_15d=slope_15d,
        annualized_slope_5d=annualized_5d,
        annualized_slope_10d=annualized_10d,
        annualized_slope_15d=annualized_15d,
        trend_r2_15d=r2_15d,
        trend_quality_15d=annualized_15d * r2_15d,
        signed_efficiency_ratio_10d=efficiency,
        trend_acceleration=annualized_5d - annualized_15d,
