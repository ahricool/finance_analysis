...
`trend_following/service.py` 对固定 CN/US Universe 计算特征、排名和纯趋势状态（IDLE / WATCHING / CANDIDATE / TRENDING / WEAKENING / BROKEN），不维护理论持仓或交易动作。大部分标的 DB-only；CN CSI2000 与 benchmark 可用 `db_fresh` 只读补尾部，远程结果不落库。支持从历史快照向后重建，因此变更规则时要考虑旧日期重算和 invalidate 行为。
