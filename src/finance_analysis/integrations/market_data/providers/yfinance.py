...
from finance_analysis.integrations.market_data.base import BaseFetcher, DataFetchError, STANDARD_COLUMNS, is_bse_code
from finance_analysis.integrations.market_data.realtime_types import UnifiedRealtimeQuote, RealtimeSource
from finance_analysis.integrations.market_data.providers.us_index_mapping import get_us_index_yf_symbol, is_us_stock_code
from finance_analysis.core.logging import log_external_call_exception

# 可选导入本地股票映射补丁，若缺失则使用空字典兜底
try:
    from finance_analysis.stocks.reference_data.mapping import STOCK_NAME_MAP, is_meaningful_stock_name
...
class YfinanceFetcher(BaseFetcher):
    ...
    def _get_us_stock_quote_from_stooq(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        request = Request(
            url,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; FinanceAnalysis/1.0; +https://github.com/ZhuLinsen/daily_stock_analysis)",
