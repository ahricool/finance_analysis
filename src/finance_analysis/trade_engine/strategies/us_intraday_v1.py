# -*- coding: utf-8 -*-
"""US holdings-related intraday warnings from shared MarketContext. Not a scanner."""
...
KEY = "us_intraday_v1"
VERSION = "1"
STRESS = {"panic", "cold", "risk_off", "divergent"}
...
class USIntradayV1:
    ...
    def evaluate(
        ...
        breadth = market_context.breadth or {}
        positive = breadth.get("member_positive_ratio_5d")
        if isinstance(positive, (int, float)) and positive < 0.4:
            reasons.append("市场宽度偏弱")
        if not reasons:
            state["last_regime"] = regime
            return []
        day = None if market_context.trading_date is None else market_context.trading_date.date().isoformat()
        signal_key = f"{KEY}:{position.position_id}:{day}:{regime}:{reasons[0]}"
        ...
        return [
            TradeSignal(
                ...
                reason=";".join(reasons),
                evidence={"regime": regime, "breadth": breadth, "warnings": list(market_context.warnings)},
