...
class MarketCommand(BotCommand):
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        """执行大盘复盘命令"""
        config = self._get_market_review_config()
        try:
            override_region = self._compute_market_review_override_region(config)
            if override_region == "":
                return BotResponse.markdown_response("🎯 大盘复盘\n\n今日相关市场休市，已跳过大盘复盘。")

            from finance_analysis.tasks.payloads import bot_message_to_payload
            from finance_analysis.tasks.queue import DuplicateTaskError, get_task_queue
