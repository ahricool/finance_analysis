...
This module is retained only as migration reference for the former Bot
ThreadPoolExecutor task service. Active business task submission now lives in
...
class TaskService:
    ...
    def submit_analysis(
        ...
        task_id = f"{code}_{utc_now().strftime('%Y%m%d_%H%M%S_%f')}"
    ...
    def _run_analysis(
        ...
        with self._tasks_lock:
            self._tasks[task_id] = {
                ...
                "code": code,
                "status": "running",
                "start_time": utc_isoformat(utc_now()),
                "result": None,
                "error": None,
                "report_type": report_type.value
            }
            ...
            if result and result.success:
                ...
                with self._tasks_lock:
                    self._tasks[task_id].update({
                        "status": "completed",
                        "end_time": utc_isoformat(utc_now()),
                        "result": result_data
                    })
