...
const props = defineProps<{ history: DetailChartPoint<ETFMomentumSnapshot>[] }>();
...
function option(series: Array<{ name: string; values: Array<number | null> }>, inverse = false) {
  return {
    ...
    xAxis: { type: 'category', data: ordered.value.map(chartDate), axisLabel: { fontSize: 9 } },
    yAxis: { type: 'value', inverse, scale: true, axisLabel: { fontSize: 9 } },
    series: series.map(item => ({ name: item.name, type: 'line', showSymbol: true, symbol: 'circle', symbolSize: 0, connectNulls: false,
      data: item.values.map((value, index) => ordered.value[index]?.isPreview
        ? { value: value ?? null, symbol: 'emptyCircle', symbolSize: 8 } : value ?? null) })),
  };
}
const priceOption = computed(() => option([
  ...
  { name: 'MA20', values: ordered.value.map(row => row.referencePrice == null || row.ma20Ratio == null ? null : row.referencePrice / (1 + row.ma20Ratio)) },
]));
