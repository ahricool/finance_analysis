...
const props = withDefaults(
  defineProps<{ variant?: AppBadgeVariant; class?: HTMLAttributes['class'] }>(),
  { variant: 'default', class: '' },
);
const baseVariant = computed(() =>
  ['default', 'secondary', 'outline', 'destructive'].includes(props.variant)
...
<template>
  <Badge
    :variant="baseVariant"
    :class="cn(semanticClass, props.class)"
  >
    <slot />
  </Badge>
</template>
