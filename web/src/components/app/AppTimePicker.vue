...
const makeOptions = (count: number, step = 1) =>
  Array.from({ length: Math.ceil(count / step) }, (_, index) => {
    const value = String(index * step).padStart(2, '0');
    return { value, label: value };
  });

const hours = makeOptions(24);
const minutes = computed(() => makeOptions(60, Math.max(1, props.minuteStep)));
const seconds = makeOptions(60);

function sync(value?: string) {
  const parts = (value || '00:00:00').split(':');
  [hour.value, minute.value, second.value] = [parts[0] || '00', parts[1] || '00', parts[2] || '00'];
}

watch(() => props.modelValue, sync, { immediate: true });
watch(open, (value) => {
  if (value) sync(props.modelValue);
});

function confirm() {
  const value = props.withSeconds
  ...
  <div :class="['grid min-w-0 gap-2', props.class]">
    ...
    <div class="flex min-w-0 gap-1">
      <Popover v-model:open="open">
        ...
        <PopoverContent
          align="start"
          class="w-[min(22rem,calc(100vw-1rem))] p-4"
        >
          <div
