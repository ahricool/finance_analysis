...
<template>
  ...
  >
    ...
    <p
      v-if="item.summary"
      class="mt-1.5 line-clamp-4 break-words text-sm leading-relaxed text-muted-foreground"
    >
      {{ item.summary }}
    </p>
    <p class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] leading-4">
      <span
        v-if="item.impact"
        class="font-medium"
