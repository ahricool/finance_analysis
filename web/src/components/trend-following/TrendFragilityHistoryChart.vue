...
const props = defineProps<{ history: DetailChartPoint<Pick<TrendSnapshot, 'tradeDate' | 'fragilityScore'>>[] }>();
...
<template>
  ...
  >
    ...
    <p class="mt-1 text-xs text-muted-foreground">
      Fragility · 截至所选交易日最近 {{ history.length }} 个展示点（含可能的 Preview） · 分数越高越脆弱
    </p>
    <div
      v-if="hasRanks"
