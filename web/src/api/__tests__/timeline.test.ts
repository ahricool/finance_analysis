...
it('sends the cutoff date and calendar type as plain query parameters', async () => {
  ...
  await timelineApi.list({ end_date: '2026-09-30', category: 'event', calendar_type: 'earnings', importance: 'critical', market: 'US' });
  expect(apiClient.get).toHaveBeenCalledWith('/api/v1/timeline', {
    params: { end_date: '2026-09-30', category: 'event', calendar_type: 'earnings', importance: 'critical', market: 'US', timezone: 'Asia/Shanghai' },
  });
});
