...
const snapshot = {
  strategy_key: 'btc_breakout_v1', symbol: 'BTCUSDT', evaluated_at: '2026-09-01T03:15:00Z', regime: 'BULL', setup: 'BREAKOUT', action: 'BUY',
...
for (const width of [1280, 1440, 1920]) {
  test(`BTC candles render and update through Binance WS at ${width}px`, async ({ page }, testInfo) => {
    ...
    await page.route('**/api/v1/**', async route => {
      ...
      if (path.endsWith('/auth/status')) body = { loggedIn: true, user: { uid: 1, role: 'user', username: 'Tester', extra: {} } };
      else if (path.endsWith('/crypto/btc/strategies')) body = [
        { strategy_key: 'btc_breakout_v1', display_name: 'Breakout V1', enabled: true },
        { strategy_key: 'btc_test_v1', display_name: 'Test V1', enabled: true },
      ];
      else if (path.endsWith('/crypto/btc/strategies/performance')) body = [
        { strategy_key: 'btc_breakout_v1', display_name: 'Breakout V1', current_position: { position_pct: '1' }, total_return: '.2', annualized_return: '.21', max_drawdown: '-.1', win_rate: '.5', running_days: '365' },
        { strategy_key: 'btc_test_v1', display_name: 'Test V1', current_position: { position_pct: '0' }, total_return: '-.1', annualized_return: '-.1', max_drawdown: '-.2', win_rate: '0', running_days: '365' },
      ];
      else if (path.endsWith('/overview')) body = { symbol: 'BTCUSDT', strategy: snapshot, state: { position_state: 'LONG' } };
      else if (path.endsWith('/performance')) body = {
        strategy_key: path.includes('btc_test_v1') ? 'btc_test_v1' : 'btc_breakout_v1', display_name: 'Breakout V1', running_days: '365', annualized_return: '.21', breakeven: 0, equity_points_total: 1,
        ...
        closed_trades: 0, wins: 0, losses: 0, win_rate: null, average_trade_return: null,
        total_return: '0', max_drawdown: '0', best_trade: null, worst_trade: null,
        recent_executions: [snapshot], recent_trades: [],
        equity_curve: [{ evaluated_at: snapshot.evaluated_at, equity: '1', drawdown: '0' }],
      };
      else if (path.endsWith('/signals')) body = { items: [snapshot] };
      if (path.includes('/btc_test_v1/')) {
        if (path.endsWith('/signals')) body = { items: [{ ...snapshot, strategy_key: 'btc_test_v1', action: 'EXIT', reason: 'Test strategy exit' }] };
        if (path.endsWith('/performance')) body = { ...body, total_return: '-.1234' };
      }
    ...
    await expect(page.getByTestId('btc-strategy-comparison')).toBeVisible();
    const requestsBeforeSwitch = marketRequests.length;
    const subscriptionsBeforeSwitch = subscriptions.length;
    await page.getByRole('combobox', { name: 'Strategy', exact: true }).selectOption('btc_test_v1');
    await expect(page.getByTestId('btc-performance').getByText('-12.34%', { exact: true })).toBeVisible();
    await expect(page.getByText('Test strategy exit', { exact: true })).toBeVisible();
    expect(marketRequests).toHaveLength(requestsBeforeSwitch);
    expect(subscriptions).toHaveLength(subscriptionsBeforeSwitch);
    expect(connections).toBe(1);
    await page.getByRole('combobox', { name: 'Strategy', exact: true }).selectOption('btc_breakout_v1');
