  ...
  for (const theme of ['light', 'dark']) {
    test(`signal center ${width}px ${theme}`, async ({ page }, testInfo) => {
      ...
      await page.route('**/api/v1/**', route => {
        ...
        if (path.endsWith('/signal-center/history')) return route.fulfill({ json: [signal] });
        if (path.endsWith('/signal-center/CN/2026-09-22')) return route.fulfill({ json: signal });
      ...
      await expect(page.getByRole('dialog')).toContainText(signal.analysis.thesis);
